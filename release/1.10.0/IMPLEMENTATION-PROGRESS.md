...
| RECOVERY-01 | Recovery Hub and ERP tools localization/accessibility | in progress | Recovery Hub, Compulsion Delay, Guided ERP, and Structured Programs are code complete. Programs expose localized task/week state, 44-point controls, Calm Insights suppression, safe persistence errors, and large-text coverage. Remaining recovery tools remain |
| CONTENT-01 | Stable-ID localized OCD tracks, programs, and Y-BOCS content | in progress | Structured programs and all five OCD tracks now resolve titles, explanations, hierarchy themes, weeks, and tasks from stable IDs in all six languages without changing enrollment/task IDs. Y-BOCS content remains |
...
| AUDIT-01 | Zero reviewed mobile/shared user-facing literal candidates | in progress | Mobile-only scope correctly excludes preserved desktop and legacy desktop-shell paths; 180 true mobile/shared candidates after Structured Programs and OCD tracks |
...
- Full Flutter suite after completing Structured Programs and OCD tracks: 255 passing tests.
- Mobile/shared literal audit: 180 candidates; stable localization selector keys are covered by narrow documented allowlist rules.
